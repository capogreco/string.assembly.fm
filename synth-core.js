...
export class SynthCore {
    ...
    async loadWorklets() {
        const loadPromises = this.workletModules.map(async (module) => {
            try {
                ...
                this.log(`Warning: Failed to load worklet ${module}: ${error.message}`, 'warn');
            }
        });
    ...
    async createAudioNodes(destination) {
        ...
        try {
            this.bowedStringNode = new AudioWorkletNode(this.audioContext, 'continuous-excitation-processor', {
                numberOfInputs: 0,
                numberOfOutputs: 1,
                outputChannelCount: [1]
            });
        } catch (error) {
            this.log(`Failed to create bowed string worklet: ${error.message}`, 'error');
        ...
        try {
            this.pinkNoiseNode = new AudioWorkletNode(this.audioContext, 'ridge-rat-type2-pink-noise-generator');
    ...
    applyProgram(program) {
        ...
        if (!this.bowedStringNode) {
            this.log('Cannot apply program: bowed string worklet not available', 'error');
            return;
        }
        ...
        for (const [param, value] of Object.entries(program)) {
            if (this.bowedStringNode.parameters.has(param)) {
                this.bowedStringNode.parameters.get(param).setValueAtTime(
                    value, 
    ...
    setPower(powerOn) {
        ...
        if (this.gainNode) {
            const targetGain = powerOn ? 
                (this.currentProgram?.masterGain || this.defaultParameters.masterGain) : 0;
    ...
    startCalibrationNoise(volume = 0.2) {
        if (!this.isInitialized || !this.pinkNoiseNode || !this.calibrationGainNode) {
            this.log('Cannot start calibration: core or calibration nodes not ready', 'error');
            return false;
        }
        ...
        if (this.bowedStringNode && typeof this.bowedStringNode.port?.postMessage === 'function') {
            this.bowedStringNode.port.postMessage({ type: 'setBowing', value: false });
        }
        ...
        this.calibrationGainNode.gain.setValueAtTime(volume, this.audioContext.currentTime);
    ...
    stopCalibrationNoise() {
        if (!this.isInitialized || !this.calibrationGainNode) {
            this.log('Cannot stop calibration: core or calibration gain node not ready', 'warn');
            return;
        }
        
        this.log('Stopping calibration noise');
        this.isCalibrating = false;
        this.calibrationGainNode.gain.setValueAtTime(0, this.audioContext.currentTime);
        ...
        try {
            ...
            this.log('Error disconnecting calibration nodes (may already be disconnected)', 'warn');
        }
    }
...
export const SynthUtils = {
    ...
    generateVariation(baseProgram, variationAmount = 0.1) {
        ...
        const varyableParams = [
            'stringDamping', 'bowPosition', 'bowSpeed', 'bowForce', 
            'brightness', 'bodyResonance', 'vibratoRate', 'vibratoDepth',
            'tremoloSpeed', 'tremoloDepth', 'trillSpeed'
        ];
