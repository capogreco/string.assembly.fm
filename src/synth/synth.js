...
export class Synth {
  ...
  sendToServer(message) {
    console.log(`[${this.id}] Attempting to send message:`, message);
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      console.log(`[${this.id}] WebSocket is open, sending message`);
  ...
  async handleServerMessage(message) {
    console.log(
      `[${this.id}] handleServerMessage called with type: ${message.type}`,
      message,
    );
    switch (message.type) {
      case "active_controllers":
        console.log(`[${this.id}] Received active_controllers:`, message.data);
        // Start WebRTC connections to controllers
        for (const ctrl_id of message.data) {
          console.log(
            `[${this.id}] Checking controller ${ctrl_id}, already connected: ${this.controllers.has(ctrl_id)}`,
          );
          if (!this.controllers.has(ctrl_id)) {
            console.log(
              `[${this.id}] Calling connectToController for ${ctrl_id}`,
            );
      ...
      case "new_controller":
        console.log(
          `[${this.id}] Received new_controller for ${message.client_id}`,
        );
        // Controller joined - connect to it
        if (!this.controllers.has(message.client_id)) {
          console.log(
            `[${this.id}] Calling connectToController for new controller ${message.client_id}`,
          );
      ...
      case "controller_disconnected":
        ...
        if (controller) {
          ...
          console.log(
            `[${this.id}] Controller ${message.client_id} disconnected`,
          );
        }
        break;
  ...
  async connectToController(controllerId) {
    console.log(`[${this.id}] connectToController called for ${controllerId}`);
    ...
    console.log(`[${this.id}] Creating WebRTC offer...`);
    const offer = await pc.createOffer();
    console.log(`[${this.id}] Offer created:`, offer);
    await pc.setLocalDescription(offer);
    console.log(`[${this.id}] Local description set`);

    console.log(`[${this.id}] Sending offer to server`);
    this.sendToServer({
      type: "offer",
      source: this.id,
      target: controllerId,
    ...
    console.log(`[${this.id}] Offer send request completed`);
  }
  ...
  handleProgram(program) {
    console.log(
      `[${this.id}] Received program with ${program.parts.length} parts`,
    );
    ...
    console.log(
      `[${this.id}] Playing part ${part.id}: ${part.pitch.toFixed(1)}Hz, ${part.expression.type}`,
    );
  }
