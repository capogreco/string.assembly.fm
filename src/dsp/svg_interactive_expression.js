...
class SVGInteractiveExpression {
  ...
  createOverlay() {
    ...
    existingCanvases.forEach((canvas) => {
      console.log(
        "Removing existing expression canvas:",
        canvas.getAttribute("data-expression-canvas"),
      );
      canvas.remove();
    });
  ...
  getChordNotes() {
    ...
    const actualChordNotes = Array.from(this.chordNotes).filter((note) => {
      ...
      return (
        !relatedInfo ||
        relatedInfo.type !== "trill-target" ||
        this.expressions.has(note)
      );
    });
  ...
  updateKeyVisual(note) {
    ...
    if (!element.hasAttribute("data-original-fill")) {
      ...
      element.setAttribute(
        ...
        element.getAttribute("stroke") || "#000",
      );
    }
  ...
  render() {
    ...
    // Rendering expression indicators
    console.log(
      "Canvas dimensions:",
      this.overlay.width,
      "x",
  ...
  drawTrillIndicator(noteData, expression) {
    ...
    const targetData = this.noteMapping.find(
      (n) => n.note === expression.targetNote,
    );
    if (!targetData) {
      ctx.restore();
      return;
  ...
  drawDragFeedback() {
    ...
    const startX =
      startRect.left - svgRect.left + startRect.width / 2 + this.canvasPadding;
    const startY =
      startRect.top - svgRect.top + startRect.height / 2 + this.canvasPadding;
    ...
    if (Math.abs(dx) > this.HORIZONTAL_THRESHOLD) {
      const targetNote = this.getNoteFromPosition(
    ...
    if (
      depth > 0 &&
      (dy < this.VIBRATO_THRESHOLD || dy > this.TREMOLO_THRESHOLD)
    ) {
  ...
  setChordNotes(notes) {
    ...
    for (const [note, expression] of this.expressions) {
      if (
        this.chordNotes.has(note) &&
        expression.type === "trill" &&
