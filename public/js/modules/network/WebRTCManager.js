...
export class WebRTCManager {
  ...
  async handleOffer(message) {
    ...
    try {
      ...
      try {
        ...
        console.log(`[WEBRTC-DEBUG] Answer created successfully for ${peerId}`);
      } catch (e) {
        if (window.Logger) {
          window.Logger.log(
  ...
  async processIceCandidateQueue(peerId) {
    ...
    if (queue.length === 0) {
      ...
      // console.log(`[WEBRTC-DIAG] Peer ${peerId}: ICE candidate queue is empty. Nothing to process.`);
      return;
    }
  ...
  async processIceCandidateQueue(peerId) {
    ...
    if (!pc.remoteDescription || !pc.remoteDescription.type) {
      console.warn(
        `[WEBRTC-DIAG] Peer ${peerId}: processIceCandidateQueue called, but remoteDescription is not yet set. Candidates will remain queued. SignalingState: ${pc.signalingState}`,
      );
      return; // Do not process if remote description isn't set.
    }

    console.log(
      `[WEBRTC-DIAG] Peer ${peerId}: Processing ${queue.length} queued ICE candidate(s). SignalingState: ${pc.signalingState}`,
    );

    // Process a copy and clear the original queue
    const candidatesToProcess = [...queue];
    peerData.iceCandidateQueue = [];
    ...
    for (const candidateData of candidatesToProcess) {
      try {
        ...
        if (window.Logger) {
          window.Logger.log(
            `Error adding queued ICE candidate from ${peerId}: ${error}`,
            "error",
          );
        }
        // Optionally, re-queue if it's a specific type of error, or discard.
        // For now, we discard on error.
      }
    }
    console.log(
      `[WEBRTC-DIAG] Peer ${peerId}: Finished processing ICE candidate queue.`,
    );
  }
  ...
  handleConnectionStateChange(pc, peerId) {
    ...
    // console.log(`[WEBRTC-DIAG] Peer ${peerId}: handleConnectionStateChange. New state: ${state}`);
  ...
  handleIceConnectionStateChange(pc, peerId) {
    ...
    switch (state) {
      ...
      case "completed":
        ...
        console.log(`[WEBRTC-DIAG] Peer ${peerId}: ICE successfully ${state}`);
