...
export const ConfigUtils = {
  ...
  getProgramParameters() {
    ...
    return Object.keys(SystemConfig.parameters).filter(name => {
      ...
      if (['transitionDuration', 'transitionStagger', 'transitionDurationSpread'].includes(name)) {
        return false;
      }
  ...
  getDefaultProgram() {
    const program = {};
    for (const [paramName, paramDef] of Object.entries(SystemConfig.parameters)) {
  ...
  validateParameter(paramName, value) {
    ...
    if (paramDef.type === 'int') {
      if (!Number.isInteger(value) || value < (paramDef.min || 0) || value > (paramDef.max || 1)) {
        return { valid: false, error: `${paramName} must be integer between ${paramDef.min || 0} and ${paramDef.max || 1}` };
      }
  ...
  clampParameter(paramName, value) {
    ...
    if (paramDef.type === 'int') {
      return Math.round(Math.max(paramDef.min || 0, Math.min(paramDef.max || 1, value)));
    } else if (paramDef.type === 'float') {
