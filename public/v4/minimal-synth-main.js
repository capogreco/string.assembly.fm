...
// This version uses the main app's WebSocketManager and shared EventBus (unchanged from V3)
...
function log(message, ...args) {
    ...
    Logger.log(`[V4-SYNTH] ${message}`, 'lifecycle');
}
...
function setupDataChannel() {
    dataChannel.onopen = () => {
        ...
        // Send initial message in JSON format expected by WebRTCManager
        const initialMessage = {
            type: 'test',
            content: 'Hello from synth!',
            timestamp: Date.now()
        };
...
async function initialize() {
    log('V4 Minimal Synth starting...');
    log('Using main app WebSocketManager and shared EventBus (unchanged from V3)');
